...
def get_version_json(minecraft_dir, version):
    """
    获取指定 Minecraft 版本的 version.json
    """
    path = os.path.join(minecraft_dir, "versions", version, f"{version}.json")
    ...
    for v in manifest["versions"]:
        if v["id"] == version:
            ver_url = v["url"]
            data = requests.get(ver_url).json()
            os.makedirs(os.path.dirname(path), exist_ok=True)
            with open(path, "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
            return data

    raise FileNotFoundError(f"找不到版本 {version} 的 JSON")
